...
fn translate_claude_permission_allow(raw: &str) -> Option<String> {
    // Bare `Tool` / `*` allow entries are intentionally not converted into
    // `capability:*` policy rules. They are too broad for a fail-closed layer
    // and remain useful through `claude_gommage_matcher`.
    if !raw.contains('(') {
        return None;
    }
    translate_claude_permission_specifier(raw)
