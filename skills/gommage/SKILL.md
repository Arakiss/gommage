...
gommage run codex --dry-run --json -- "<task>"
gommage doctor --json
gommage smoke --json
gommage stats --json
...
- Exact-input approvals: `bind_input: true` is valid only with `decision: ask_picto`. It makes approval mint a picto tied to the canonical observed tool input as well as scope. Direct `gommage grant --scope …` remains scope-only; approve the pending request from an input-bound rule when the exact call matters.
