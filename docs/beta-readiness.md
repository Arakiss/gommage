...
| Operator TUI | `gommage tui --snapshot --view all` shows summary, focus, readiness rows, approvals, policy, audit, capability, recovery, and next actions on a clean pre-init home and after quickstart. `gommage tui --watch --watch-ticks 2 --view approvals` produces bounded plain-text refreshes without ANSI escapes. `gommage tui --stream --stream-ticks 1` shows recent decision/event rows through daemon IPC when available and falls back to signed audit log reads. |
