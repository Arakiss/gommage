...
After installing an integration, run `gommage doctor --json` to verify the operator path and `gommage smoke --json` to verify mapper + policy semantics. If the repository carries policy fixtures, also run `gommage policy test <file> --json` before trusting the hook. A top-level doctor `warn` is still an operable install when the only warnings are the missing first audit log or missing daemon socket. A top-level doctor `fail`, smoke `fail`, or policy-test `fail` means the hook should not be trusted yet. See [`diagnostics.md`](diagnostics.md).
